...
fn main() -> Result<()> {
    ...
    //
    // Another way to see the difference is to rename the unused variable below `_try_rename_this` to `_`, which will have the same effect
    let sbox = usandbox
    ...
    let (_try_rename_this, res) = sbox.call_guest_function_by_name(
        "Echo",
        ReturnType::String,
