...
use crate::{func::types::ReturnType, Sandbox, UninitializedSandbox};
...
impl<'a> ExposedMethods<'a> for UninitializedSandbox<'a> {
    fn guest_method(sbox: &mut Sandbox<'a>, a1: String) -> i32 {
        sbox.call_dynamic_guest_function("guest_method", ReturnType::Int, Some(vec![a1]))
            .expect("failed to call guest function")
    }

    fn print_output(sbox: &mut Sandbox<'a>, a1: String) -> i32 {
        sbox.call_dynamic_guest_function("print_output", ReturnType::Int, Some(vec![a1]))
            .expect("failed to call guest function")
    }
