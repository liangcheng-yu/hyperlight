...
{
    public static class NativeHandleWrapperFunctionCallResultExtensions
    {
        public static bool IsFunctionCallResult(this Handle hdl)
        {
            HyperlightException.ThrowIfNull(hdl, MethodBase.GetCurrentMethod()!.DeclaringType!.Name);
            return handle_is_function_call_result(hdl.ctx.ctx, hdl.handle);
        }
        public static FunctionCallResult GetFunctionCallResult(this Handle hdl)
        {
            HyperlightException.ThrowIfNull(hdl, MethodBase.GetCurrentMethod()!.DeclaringType!.Name);
            var bufferPtr = handle_get_function_call_result_flatbuffer(hdl.ctx.ctx, hdl.handle);
            ...
            var functionCallResult = FunctionCallResult.GetRootAsFunctionCallResult(byteBuffer);
            function_call_result_flatbuffer_free(bufferPtr);
