...
mkdir -p src/tests/Guests/dummyguest/x64/debug/ && cd  src/tests/Guests/dummyguest/x64/debug/ && gh release download  ${RELEASE_TAG} -p 'dummyguest.exe' --clobber
cd $REPO_DIR
mkdir -p src/tests/Guests/dummyguest/x64/release/ && cd  src/tests/Guests/dummyguest/x64/release/ && gh release download  ${RELEASE_TAG} -p 'dummyguest.exe' --clobber
cd $REPO_DIR
mkdir -p src/tests/Guests/simpleguest/x64/debug/ && cd  src/tests/Guests/simpleguest/x64/debug/ && gh release download  ${RELEASE_TAG} -p 'simpleguest.exe' --clobber
cd $REPO_DIR
mkdir -p src/tests/Guests/simpleguest/x64/release/ && cd  src/tests/Guests/simpleguest/x64/release/ && gh release download  ${RELEASE_TAG} -p 'simpleguest.exe' --clobber
cd $REPO_DIR
